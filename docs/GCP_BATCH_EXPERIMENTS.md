...
```bash
export BUCKET_NAME="${PROJECT_ID}-kuhn-poker-results"
...
```bash
export SA_NAME="kuhn-experiment-runner"
export SA_EMAIL="${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com"

gcloud iam service-accounts create "$SA_NAME" \
  --display-name="Kuhn poker experiment runner" \
  --project="$PROJECT_ID"
```
...
export PROJECT_ID="your-gcp-project-id"
export REGION="europe-west1"
export BUCKET="gs://${PROJECT_ID}-kuhn-poker-results"
export SA_EMAIL="kuhn-experiment-runner@${PROJECT_ID}.iam.gserviceaccount.com"
...
git clone --depth 1 https://github.com/lawrencewlcknight/kuhn-poker-deep-cfr-experiments.git
cd kuhn-poker-deep-cfr-experiments
...
python3 -m venv --copies /tmp/kuhn-venv
source /tmp/kuhn-venv/bin/activate
...
The script prints the Batch script before submission. Check that the upload line uses `gsutil`, for example:
...
./gcp/submit_batch_experiment.sh \
  "exp1-validation-$(date +%Y%m%d-%H%M%S)" \
  "/usr/bin/time -v python -m experiments.kuhn_poker.deep_cfr_multiseed_validation.run \
    --output-root outputs/cloud/exp1-validation" \
  "n2-standard-4" \
  "43200" \
  "4000" \
...
The script deliberately creates a virtual environment under `/tmp/kuhn-venv` rather than installing packages into the system Python. This is important because installing experiment dependencies into the system Python can interfere with the Google Cloud CLI on the Batch VM.
...
The script uses `gsutil` rather than `gcloud storage cp` because `gsutil` proved more reliable on the default Batch image for this workflow.
