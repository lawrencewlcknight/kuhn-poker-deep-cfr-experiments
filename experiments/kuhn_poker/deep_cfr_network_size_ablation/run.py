"""CLI for the Kuhn poker Deep CFR network-size ablation."""
...
from .plotting import plot_network_size_ablation  # noqa: E402


_LOGGER = logging.getLogger("deep_cfr_poker.experiment.network_size")
...
def _variants_from_grid(depths: Sequence[int], widths: Sequence[int]) -> list[dict]:
    return [
...
def build_config(args) -> dict:
    ...
    depths = parse_int_list(args.depths) or list(NETWORK_DEPTHS)
    widths = parse_int_list(args.widths) or list(NETWORK_WIDTHS)
    variant_source = (
        _variants_from_grid(depths, widths)
        if args.depths is not None or args.widths is not None
        else NETWORK_SIZE_VARIANTS
    )
    variants = _filter_variants(variant_source, parse_variant_ids(args.variant_ids))
...
def _augment_result(
    ...
    result["config_label"] = variant_id
    result["architecture_depth"] = int(variant_config["architecture_depth"])
    result["architecture_width"] = int(variant_config["architecture_width"])
    ...
    variant_fields = {
        ...
        "architecture_depth": int(variant_config["architecture_depth"]),
        "architecture_width": int(variant_config["architecture_width"]),
...
def _export_ablation_npz(
    ...
    for variant in variants:
        ...
        for diag_key in (
            ...
            payload[f"{variant_id}_{diag_key}"] = _stack_padded(
                [
                    np.asarray(r["diagnostics"][diag_key], dtype=np.float64)
                    for r in subset
                    if diag_key in r["diagnostics"]
                ]
            )
...
def _architecture_grid_rows(
    ...
    for variant in variants:
        ...
        rows.append(
            {
                "variant_id": variant_id,
                "variant_label": str(variant.get("label", variant_id)),
                "architecture_depth": int(variant["architecture_depth"]),
                "architecture_width": int(variant["architecture_width"]),
                "policy_network_layers": tuple(variant["policy_network_layers"]),
                "advantage_network_layers": tuple(variant["advantage_network_layers"]),
                ...
                "final_nodes_touched_mean": _summary_lookup(
                    summary, "final_nodes_touched", "mean"
                ),
            }
        )
    return rows
...
def export_ablation_results(
    ...
    grid_rows = _architecture_grid_rows(variants, aggregate_by_variant)
    grid_summary_csv = write_dict_rows_csv(
        grid_rows, run_dir / "architecture_grid_summary.csv"
    )
    ...
    write_experiment_metadata(
        ...
        extra={
            ...
            "architecture_grid_summary_csv": str(grid_summary_csv),
    ...
    return {
        **info,
        ...
        "architecture_grid_summary_csv": grid_summary_csv,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Kuhn poker Deep CFR network-size ablation."
    )
    ...
    parser.add_argument("--depths", default=None, help="Comma-separated hidden-layer counts.")
    parser.add_argument("--widths", default=None, help="Comma-separated hidden-layer widths.")
...
def main() -> int:
    ...
    _LOGGER.info("Architecture summary: %s", export_info["architecture_grid_summary_csv"].resolve())
