...
This experiment tests two implementation choices in the shared Deep CFR solver:
how advantage-memory samples are drawn for advantage-network training, and how
average-strategy samples are weighted when training the average-policy network.

The default configuration is aligned with the Experiment 2 baseline. The only
treatment variables are:

- `advantage_replay_sampling`: `uniform` or `priority_abs_adv`
